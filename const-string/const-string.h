...
namespace LL {
    ...
    template<int N, int ...S>
    constexpr
    auto Unpack(const char (&s)[N], Sequence<S...>)
        -> const_str<s[S]...>
    {
    ...
    inline constexpr
    auto operator "" _const(const char * val, size_t sz)
    ...
    constexpr inline
    T_size load_word(byte B0, byte B1, byte B2, byte B3)
    {
        return static_cast<T_size>(  (B3<<(8*3))
                                   | (B2<<(8*2))
                                   | (B1<<(8*1))
                                   | (B0<<(8*0)) );
    }
    ...
    template <class T_size>
    constexpr inline
    T_size load_word(const byte * _B_ptr)
    {
        //    return static_cast<const _Size>( *static_cast<const _Size*>( _B_ptr ) );
        return static_cast<T_size>(  (_B_ptr[0]<<(8*0))
                                   | (_B_ptr[1]<<(8*1))
                                   | (_B_ptr[2]<<(8*2))
                                   | (_B_ptr[3]<<(8*3)) );
    }
